...
void eval(char *cmdline) 
{
  ...
  if(cmd2 == NULL)//only one command
  {
    if(!builtin_cmd(argv1))//not in built-in command
    {
      sigprocmask(SIG_BLOCK, &one, NULL);//block SIGCHLD
      if((pid = fork()) == 0)//in child
      {
        sigprocmask(SIG_SETMASK, NULL, NULL);//unblock 
        if(bg)
      ...
      sigprocmask(SIG_SETMASK, NULL, NULL);//unblock all
      if(!bg)
        waitfg(pid);
      else
...
void waitfg(pid_t pid)
{
  ...
  while(pid == fgpid(jobs))//chech if the foreground job has finished //????????????
    sigsuspend(&one);
...
void sigchld_handler(int sig) 
{
  ...
  while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
  {
    ...
    {
      if((child_sig = WTERMSIG(status)) == SIGINT)//??????????????????
...
/* fgpid - Find PID of current foreground job */
pid_t fgpid(struct job_t *jobs)
{
  int i;
